...
def create_firehose_stream(firehose, stream_name, role_arn, bucket_arn):
...
def create_role(iam, policy_name, assume_role_policy_document, policy_str):
    ...
    if role_exists(iam, policy_name):
        logging.info('Role "{0}" already exists. Assuming correct values.'.format(policy_name))
        ...
        logging.info('response for creating role = "%s"',
                     response)
        return response['Role']['Arn']

def create_lambda_function(lambda_client, function_name, runtime, role, handler, zip_file, description, timeout, mem_size):
